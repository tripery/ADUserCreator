...
function Split-FullName {
    ...
    if ($parts.Count -lt 2) { throw "Unable to parse fullName '$FullName'. Expected at least surname and given name." }
    [pscustomobject]@{
        Surname    = $parts[0]
        GivenName  = $parts[1]
...
function Build-BaseIdentifiers {
    ...
    [pscustomobject]@{
        SurnameLatin = $surnameLat
        GivenLatin   = $givenLat
        MiddleLatin  = $middleLat
        SamBase      = $samBase
        MailLocalBase = $samBase
    }
}
...
function Invoke-AdUserCreate {
    ...
    foreach ($groupSam in $GroupsToAdd) {
        if ($groupSam) { Add-ADGroupMember -Identity $groupSam -Members $Preview.login -ErrorAction Stop }
    }
    [pscustomobject]@{
    ...
    try {
        switch ("$method $path") {
            ...
            'POST /api/users/preview' {
                ...
                if ([string]::IsNullOrWhiteSpace($domainSuffix)) { throw 'domainSuffix is required.' }
                $users = @($body.users)
            ...
            'POST /api/users/create' {
                ...
                $ou = Normalize-Text $body.ou
                if ([string]::IsNullOrWhiteSpace($domainSuffix)) { throw 'domainSuffix is required.' }
                if ([string]::IsNullOrWhiteSpace($ou)) { throw 'ou is required.' }
...
try {
    ...
    throw "Failed to start HttpListener on http://+:$Port/. Configure URL ACL first: $urlAclCmd"
}
...
Write-Host 'NOTE: backend uses inline preview/create logic for the current src layout.'
